...
import ui.pages.ClientAddsDelli7Page;
import ui.pages.ClientAddsSonyVaioI5Page;
import ui.utilities.ConfigReader;
import ui.utilities.Driver;
import ui.utilities.JSUtilities;
...
public class ClientAddsSonyStepDefs {
    ...
    public void user_navigates_to_the_home_page() {
        Driver.getDriver().get(ConfigReader.getProperty("demo_blaze_url"));
    }
    @When("user clicks on Laptops button")
    public void user_clicks_on_laptops_button() {
        dell.home.click();
        ReusableMethods.waitForVisibility(client.laptopsButton,10);
    ...
    public void user_clicks_on_sony_vaio_i5() throws InterruptedException {
        Thread.sleep(2000);
        JSUtilities.scrollToElement(client.sonyVaio);
        client.sonyVaio.click();
    }
    @Then("user clicks on Add to cart button")
    public void user_clicks_on_add_to_cart_button() throws InterruptedException {
        ReusableMethods.waitForVisibility(client.addToCartButton,15);
    ...
    public void user_fill_in_all_web_form_fields() throws InterruptedException {
        Thread.sleep(2000);
        dell.name.click();
        ReusableMethods.waitAndSendText(dell.name,"isabelle",2);
        ReusableMethods.waitAndSendText(dell.country,"Spain",2);
        ReusableMethods.waitAndSendText(dell.city,"Madrid",2);
        ReusableMethods.waitAndSendText(dell.creditCard,"1234",2);
        ReusableMethods.waitAndSendText(dell.month,"May",2);
        ReusableMethods.waitAndSendText(dell.year,"2023",2);

    }
    @Then("user clicks on purchase button")
    public void user_clicks_on_purchase_button() throws InterruptedException {
        expectedamount=dell.expectedamount.getText()+" USD";
        Thread.sleep(2000);
        dell.purchase.click();
